...
class LocalFeatureTransformer(nn.Module):
    """Modified base on LoFTR's LocalFeatureTransformer."""
    def __init__(self, config):
        ...
        self.layer_names = config['layer_names'].split('-')
    ...
    def forward(self, feats, masks=None):
        ...
        feats_i = [x.squeeze(1) for x in feats.split(1, dim=1)]
        for layer, name in zip(self.layers, self.layer_names):
            if name == 'self':
                for i in range(n):
                    feats_i[i] = layer(feats_i[i], feats_i[i], None, None)
            elif name == 'cross':
                feats = torch.cat(feats_i, dim=1)
                feats = layer(feats, feats, None, None)
                feats_i = [x.squeeze(1) for x in feats.split(l, dim=1)]
        feats = torch.stack(feats_i, dim=1)
        return feats
